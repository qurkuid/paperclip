...
- **Continuation policy.** `request_confirmation`, `request_checkbox_confirmation`, and `request_item_verdicts` default to `wake_assignee`, which wakes you after the board resolves the decision or submits newly resolved item verdicts. For `request_confirmation`, rejection feedback therefore starts a revision attempt by default. Use `wake_assignee_on_accept` when only acceptance should resume you, and `none` only when neither outcome should resume you.
